// 🔥 (avvia allenamento, obiettivo settimana, HIIT) — pixellata da twemoji, CC-BY 4.0 — attribuzione richiesta
// generato da scripts/png2icona.py — non si modifica a mano:
// si ridisegna il PNG e si rilancia lo script
export default function Energia(props) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" shapeRendering="crispEdges">
    <g fill="var(--ico-a, #f38f0c)">
    <rect x="14" y="1" width="2" height="1"/>
    <rect x="7" y="2" width="2" height="3"/>
    <rect x="14" y="2" width="3" height="2"/>
    <rect x="14" y="4" width="4" height="1"/>
    <rect x="7" y="5" width="3" height="1"/>
    <rect x="13" y="5" width="5" height="1"/>
    <rect x="2" y="6" width="1" height="2"/>
    <rect x="7" y="6" width="11" height="4"/>
    <rect x="1" y="8" width="2" height="2"/>
    <rect x="1" y="10" width="3" height="1"/>
    <rect x="6" y="10" width="12" height="1"/>
    <rect x="22" y="10" width="1" height="2"/>
    <rect x="1" y="11" width="10" height="3"/>
    <rect x="13" y="11" width="4" height="1"/>
    <rect x="14" y="12" width="4" height="1"/>
    <rect x="21" y="12" width="2" height="1"/>
    <rect x="15" y="13" width="8" height="3"/>
    <rect x="1" y="14" width="4" height="2"/>
    <rect x="7" y="14" width="4" height="1"/>
    <rect x="7" y="15" width="3" height="1"/>
    <rect x="1" y="16" width="3" height="1"/>
    <rect x="8" y="16" width="1" height="1"/>
    <rect x="16" y="16" width="2" height="2"/>
    <rect x="20" y="16" width="3" height="1"/>
    <rect x="2" y="17" width="2" height="2"/>
    <rect x="20" y="17" width="2" height="2"/>
    <rect x="3" y="19" width="1" height="1"/>
    <rect x="20" y="19" width="1" height="1"/>
    </g>
    <g fill="var(--ico-b, #ffcc4d)">
    <rect x="11" y="11" width="1" height="1"/>
    <rect x="11" y="12" width="3" height="1"/>
    <rect x="12" y="13" width="2" height="1"/>
    <rect x="11" y="14" width="4" height="2"/>
    <rect x="5" y="15" width="2" height="2"/>
    <rect x="10" y="16" width="5" height="1"/>
    <rect x="4" y="17" width="11" height="1"/>
    <rect x="19" y="17" width="1" height="1"/>
    <rect x="4" y="18" width="12" height="1"/>
    <rect x="17" y="18" width="3" height="1"/>
    <rect x="4" y="19" width="16" height="2"/>
    <rect x="5" y="21" width="14" height="1"/>
    <rect x="6" y="22" width="12" height="1"/>
    <rect x="8" y="23" width="8" height="1"/>
    </g>
    <g fill="var(--ico-c, #f59714)">
    <rect x="12" y="11" width="1" height="1"/>
    <rect x="11" y="13" width="1" height="1"/>
    <rect x="14" y="13" width="1" height="1"/>
    <rect x="5" y="14" width="2" height="1"/>
    <rect x="10" y="15" width="1" height="1"/>
    <rect x="4" y="16" width="1" height="1"/>
    <rect x="7" y="16" width="1" height="1"/>
    <rect x="9" y="16" width="1" height="1"/>
    <rect x="15" y="16" width="1" height="2"/>
    <rect x="18" y="16" width="2" height="1"/>
    <rect x="18" y="17" width="1" height="1"/>
    <rect x="16" y="18" width="1" height="1"/>
    </g>
    </svg>
  )
}
